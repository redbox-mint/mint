...
appender("GEO", RollingFileAppender) {
  file = "${logHome}/logs/geonames/geonames.log"
  rollingPolicy(TimeBasedRollingPolicy) {
    fileNamePattern = "${logHome}/logs/archives/%d{yyyy-MM}/geonames/geonames.zip"
    maxHistory = 30
  }
  encoder(PatternLayoutEncoder) {
    pattern = "%d %-8X{name} %-6p %-20.20c{0} %m%n"
...
logger("au.edu.usq.adfi", DEBUG, ["GEO"])
